# <a name="documentation"></a>Hepatitis A virus (HAV) genome annotation

## [How to annotate HAV sequences with VADR](#howto)

## [HAV VADR model library](#havmodels)
...
---
## <a name="howto"></a>How to annotate HAV genomes with VADR
...
2. Clone the latest HAV VADR model library from this repository. <!-- (current release v1.0)<br/>-->
   `git clone git@github.com:greninger-lab/vadr-models-hav.git`<br/>
   <!--or download the current release from [here](https://github.com/greninger-lab/vadr-models-hpiv/releases/tag/v1.0).</br>-->
   Note the path to the directory name created plus the "hav"
   subdirectory (e.g. /path/to/vadr-models-hav/hav) as `<hav-models-dir-path>`
   for step 4.
...
4. Run the `v-annotate.pl` program on an input trimmed fasta file with
   HAV sequences using the recommended command below.
```
...
* This page was adapted for HAV from [Mpox virus annotation](https://github.com/ncbi/vadr/wiki/Mpox-virus-annotation)
